//Program to find Addition for 2 Polynomials using Single Linked List.
#include<stdio.h>
#include<stdlib.h>
typedef struct poly{
    int exp, coef;
...
//Function for input Polynomials.
node *insert(node *poly, int exp, int coef){
    node *p=NULL;
...
void addPoly(node *poly1, node *poly2){
    ...
    while(poly1 != NULL && poly2 != NULL){
        if(poly1->exp == poly2->exp){
            poly3 = insert(poly3, poly1->exp, poly1->coef+poly2->coef);
            poly1 = poly1->next;
            poly2 = poly2->next;
        }
        else if(poly1->exp > poly2->exp){
            poly3 = insert(poly3, poly1->exp, poly1->coef);
            poly1 = poly1->next;
        }
        else if(poly1->exp < poly2->exp){
            poly3 = insert(poly3, poly2->exp, poly2->coef);
            poly2 = poly2->next;
        }
        if(tmp == NULL)
            tmp = poly3;
